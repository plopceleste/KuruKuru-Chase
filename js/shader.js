const SHADERS = {};

SHADERS.lottes = {
name: 'lottes',
source: `
#define res (iResolution.xy/3.0)

float hardScan=-4.0;
float hardPix=-3.0;
vec2 warp=vec2(1.0/100.0,1.0/100.0);
float maskDark=0.85;
float maskLight=1.15;

float ToLinear1(float c){return(c<=0.04045)?c/12.92:pow((c+0.055)/1.055,2.4);}
vec3 ToLinear(vec3 c){return vec3(ToLinear1(c.r),ToLinear1(c.g),ToLinear1(c.b));}

float ToSrgb1(float c){return(c<0.0031308?c*12.92:1.055*pow(c,0.41666)-0.055);}
vec3 ToSrgb(vec3 c){return vec3(ToSrgb1(c.r),ToSrgb1(c.g),ToSrgb1(c.b));}

vec3 Fetch(vec2 pos,vec2 off,vec2 r){
  pos=floor(pos*r+off)/r;
  if(max(abs(pos.x-0.5),abs(pos.y-0.5))>0.5)return vec3(0.0,0.0,0.0);
  return ToLinear(texture(iChannel0,pos.xy,-16.0).rgb);}

vec2 Dist(vec2 pos,vec2 r){pos=pos*r;return -((pos-floor(pos))-vec2(0.5));}

float Gaus(float pos,float scale){return exp2(scale*pos*pos);}

vec3 Horz3(vec2 pos,float off,vec2 r){
  vec3 b=Fetch(pos,vec2(-1.0,off),r);
  vec3 c=Fetch(pos,vec2( 0.0,off),r);
  vec3 d=Fetch(pos,vec2( 1.0,off),r);
  float dst=Dist(pos,r).x;
  float scale=hardPix;
  float wb=Gaus(dst-1.0,scale);
  float wc=Gaus(dst+0.0,scale);
  float wd=Gaus(dst+1.0,scale);
  return (b*wb+c*wc+d*wd)/(wb+wc+wd);}

vec3 Horz5(vec2 pos,float off,vec2 r){
  vec3 a=Fetch(pos,vec2(-2.0,off),r);
  vec3 b=Fetch(pos,vec2(-1.0,off),r);
  vec3 c=Fetch(pos,vec2( 0.0,off),r);
  vec3 d=Fetch(pos,vec2( 1.0,off),r);
  vec3 e=Fetch(pos,vec2( 2.0,off),r);
  float dst=Dist(pos,r).x;
  float scale=hardPix;
  float wa=Gaus(dst-2.0,scale);
  float wb=Gaus(dst-1.0,scale);
  float wc=Gaus(dst+0.0,scale);
  float wd=Gaus(dst+1.0,scale);
  float we=Gaus(dst+2.0,scale);
  return (a*wa+b*wb+c*wc+d*wd+e*we)/(wa+wb+wc+wd+we);}

float Scan(vec2 pos,float off,vec2 r){
  float dst=Dist(pos,r).y;
  return Gaus(dst+off,hardScan);}

vec3 Tri(vec2 pos,vec2 r){
  vec3 a=Horz3(pos,-1.0,r);
  vec3 b=Horz5(pos, 0.0,r);
  vec3 c=Horz3(pos, 1.0,r);
  float wa=Scan(pos,-1.0,r);
  float wb=Scan(pos, 0.0,r);
  float wc=Scan(pos, 1.0,r);
  return a*wa+b*wb+c*wc;}

vec2 Warp(vec2 pos){
  pos=pos*2.0-1.0;
  pos*=vec2(1.0+(pos.y*pos.y)*warp.x,1.0+(pos.x*pos.x)*warp.y);
  return pos*0.5+0.5;}

vec3 Mask(vec2 pos){
  pos.x+=pos.y*3.0;
  vec3 mask=vec3(maskDark,maskDark,maskDark);
  pos.x=fract(pos.x/6.0);
  if(pos.x<0.333)mask.r=maskLight;
  else if(pos.x<0.666)mask.g=maskLight;
  else mask.b=maskLight;
  return mask;}

void mainImage( out vec4 fragColor, in vec2 fragCoord ){
  vec2 uv=fragCoord.xy/iResolution.xy;
  vec3 worldCol=ToSrgb(Tri(Warp(uv),res)*Mask(fragCoord.xy));
  vec4 ui=texture(iChannel1,uv);
  fragColor=vec4(mix(worldCol,ui.rgb,ui.a),1.0);}
`
};

const DEFAULT_SHADER = 'lottes';
const SHADER_ORDER = ['off', 'lottes'];
window.SHADERS = SHADERS;
window.DEFAULT_SHADER = DEFAULT_SHADER;
window.SHADER_ORDER = SHADER_ORDER;
